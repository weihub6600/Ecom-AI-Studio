...
'@
    ...
    Write-Host ""
    Write-Host "文件更新完成，正在执行项目构建……" -ForegroundColor Cyan
    ...
    foreach ($RelativeFile in $PowerShellFiles) {
    ...
    try {
        ...
        Write-Host ""
        Write-Host "正在检查敏感文件……" -ForegroundColor Cyan

        & powershell -NoProfile -ExecutionPolicy Bypass -File ".\scripts\check-sensitive-files.ps1" -ProjectRoot $ProjectRoot
        ...
        Write-Host ""
        Write-Host "正在安装 Git 提交保护……" -ForegroundColor Cyan

        & powershell -NoProfile -ExecutionPolicy Bypass -File ".\scripts\install-git-hooks.ps1" -ProjectRoot $ProjectRoot
    ...
    Write-Host " V11 安装成功" -ForegroundColor Green
    Write-Host "============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "V10 备份目录：$BackupRoot"
    Write-Host ""
    Write-Host "启动项目："
    ...
    Write-Host ""
    Write-Host "备份 MySQL："
    Write-Host "  npm run backup:mysql"
    Write-Host ""
}
catch {
    Write-Host ""
    Write-Host "V11 安装失败。" -ForegroundColor Red
    Write-Host $_.Exception.Message -ForegroundColor Red
    Write-Host ""
    Write-Host "正在自动恢复 V10 文件……" -ForegroundColor Yellow

    if (Test-Path -LiteralPath $BackupRoot) {
        Restore-ProjectFiles
    }

    Write-Host "V10 文件已经恢复。" -ForegroundColor Yellow
