...
- 灵客、GRSAI、Nano Banana 可在站长后台直接编辑
- Base URL、接口路径、API Key、超时和启停保存到 MySQL
- 内置模型的 API 模型 ID、名称、说明、尺寸、参考图能力和积分可编辑
- API Key 使用 AES-256-GCM 加密，前端只显示末四位
- `.env` 仅用于首次迁移和故障回退

## V13.7.3 ZHE AI、验证码与注册审核开关

- 登录和注册均增加服务端图形验证码
- 站长后台可控制新用户注册是否需要审核
- 验证码保存于云端数据库，5分钟有效且单次使用
- 百嘉瑞AI、BJR AI 统一更名为 ZHE AI
- BJR 0.1 升级为 ZHE 1.0
- 生成历史的用户文案由 MySQL 改为云端
- 内置服务商和模型继续支持站长后台可视化编辑
