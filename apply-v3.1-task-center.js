...
const backupDir = path.join(root, ".v3.1-task-center-backup");
const touched = [
  "web/src/App.vue",
  "web/src/style.css",
  "web/src/types.ts",
...
function replaceRegexOnce(text, pattern, to, label) {
  if (!pattern.test(text)) throw new Error(`无法定位修改位置：${label}`);
  pattern.lastIndex = 0;
...
function patchTypes() {
  let s = read("web/src/types.ts");
  const usageStart = s.indexOf("export interface UsageRecord {");
  if (usageStart < 0) throw new Error("无法定位修改位置：UsageRecord 类型");
  const usageEnd = s.indexOf("}\n", usageStart);
  if (usageEnd < 0) throw new Error("无法定位修改位置：UsageRecord 类型结尾");
  let block = s.slice(usageStart, usageEnd + 2);
  if (!block.includes("requestId?: string;")) {
    block = block.replace(
      "  pointsRefunded?: boolean;",
      "  pointsRefunded?: boolean;\n  requestId?: string;\n  operationId?: string;"
    );
  }
  s = s.slice(0, usageStart) + block + s.slice(usageEnd + 2);
  write("web/src/types.ts", s);
}
...
function patchApp() {
  ...
  if (!/\bUsageRecord\b/.test(s.slice(s.indexOf('import type {'), s.indexOf('} from "./types";') + 18))) {
    s = replaceRegexOnce(
      s,
      /(\s+UploadImage)(\s*\n}\s+from\s+"\.\/types";)/,
      "$1,\n  UsageRecord$2",
  ...
  if (!s.includes("await Promise.all([loadHistory(true), loadTasks()]);")) {
    s = replaceRegexOnce(
      s,
      /if \(authUser\.value\) await loadHistory\(true\);\s*\n}\);/,
      "if (authUser.value) {\n    await Promise.all([loadHistory(true), loadTasks()]);\n    startTaskRefresh();\n  }\n});",
      "启动时读取任务"
    );
  }

  const authStart = s.indexOf("async function handleAuthenticated(");
  const authEnd = authStart >= 0 ? s.indexOf("\n}", authStart) : -1;
  if (authStart >= 0 && authEnd >= 0) {
    let block = s.slice(authStart, authEnd + 2);
    if (!block.includes("loadTasks()")) {
      block = block.replace(
        "  await loadHistory(true);",
        "  await Promise.all([loadHistory(true), loadTasks()]);\n  startTaskRefresh();"
      );
      s = s.slice(0, authStart) + block + s.slice(authEnd + 2);
    }
  } else {
    throw new Error("无法定位修改位置：登录后读取任务");
  }
...
`;
  ...
  const generateStart = s.indexOf("async function generate()");
  const generateEnd = generateStart >= 0 ? s.indexOf("async function pollGenerationTask", generateStart) : -1;
  if (generateStart < 0 || generateEnd < 0) {
    throw new Error("无法定位修改位置：生成流程");
  }
  let generateBlock = s.slice(generateStart, generateEnd);

  if (!/handleBalanceUpdated\(response\.credits\);[\s\S]{0,180}await loadTasks\(\);/.test(generateBlock)) {
    if (!/\n\s*let result = response\.result;/.test(generateBlock)) {
      throw new Error("无法定位修改位置：提交后刷新任务");
    }
    generateBlock = generateBlock.replace(
      /\n(\s*)let result = response\.result;/,
      "\n$1await loadTasks();\n\n$1let result = response.result;"
    );
  }

  const finallyIndex = generateBlock.lastIndexOf("finally {");
  if (finallyIndex < 0) throw new Error("无法定位修改位置：生成结束逻辑");
  const finallyBlock = generateBlock.slice(finallyIndex);
  if (!finallyBlock.includes("await loadTasks();")) {
    const marker = 'pollingProgress.value = "";';
    const markerIndex = generateBlock.lastIndexOf(marker);
    if (markerIndex < finallyIndex) throw new Error("无法定位修改位置：结束后刷新任务");
    const insertAt = markerIndex + marker.length;
    generateBlock = generateBlock.slice(0, insertAt) + "\n    await loadTasks();" + generateBlock.slice(insertAt);
  }

  s = s.slice(0, generateStart) + generateBlock + s.slice(generateEnd);
...
function apply() {
  ...
  const taskComponent = p("web/src/components/TaskCenter.vue");
  const appText = read("web/src/App.vue");
  if (fs.existsSync(taskComponent) && !appText.includes("TaskCenter") && read("web/src/components/TaskCenter.vue").includes("TASK CENTER")) {
    fs.rmSync(taskComponent, { force: true });
    console.log("已清理上一次失败补丁遗留的 TaskCenter.vue");
  }
  backup();
  ...
  console.log("V3.1 任务中心补丁安装成功。");
}

try {
  if (process.argv.includes("--rollback")) {
    rollback();
    console.log("V3.1 补丁已回滚。");
  } else {
    apply();
  }
} catch (error) {
  ...
  if (!process.argv.includes("--rollback") && !process.argv.includes("--keep-on-build-failure") && fs.existsSync(backupDir)) {
    try {
      rollback();
      console.error("已自动恢复到安装 V3.1 前的文件。");
    } catch (rollbackError) {
      console.error("自动回滚失败：", rollbackError instanceof Error ? rollbackError.message : rollbackError);
    }
  }
  process.exitCode = 1;
}
